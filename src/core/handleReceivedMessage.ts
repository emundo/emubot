...
export async function handleReceivedMessageInCore(
    ...
    try {
        ...
        const messageAfterFirstInterceptor: Response<
            ChatAdapterRequest
        > = await (await chatToCore).handleMessage(messengerUserId, request);
        ...
        if (messageAfterFirstInterceptor.kind === 'Response') {
            if (
                ...
                const messageRetrievedFromInterceptorNlpToCore = await (await nlpToCore).handleMessage(
